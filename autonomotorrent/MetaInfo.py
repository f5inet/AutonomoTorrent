...
class BTMetaInfo:
    ...
    def __init__(self, path=None, meta_info=None):
        ...
        if b'announce' in metainfo:
            self.announce_list = [metainfo[b'announce']]
            if b'announce-list' in metainfo:
                self.announce_list += reduce(lambda x, y: x + y, metainfo[b'announce-list'])
        else:  # Trackerless torrent?
            self.announce_list = []

        if b'encoding' in metainfo:
            self.encoding = metainfo[b'encoding']
        ...
        self.topDir = '.'
        name = info[b'name'].decode(self.encoding)
        if b'files' in info:
            cur_size = 0
            ...
            _d = {}
            _d['path'] = name
            _d['length'] = info[b'length']
            _d['pos_range'] = 0, info[b'length']  # TODO: Is this right?
            self.files.append(_d)
